...
import numpy as np
from Jiezi.Physics.poisson import poisson, poissonGuess
...
from Jiezi.FEM import ef_solver
...
def normal(mul, mur, Dirichlet_BC_gate, weight_old, tol_loop, process_id):
    ...
    doping_source_init = 1e-5
    doping_channel_init = 0
    doping_drain_init = 1e-5
    print("init guess is:", doping_source_init, doping_drain_init)
    doping_GP_list4init = constant_parameters.doping(coord_GP_list, zlength_oxide, z_translation, z_isolation,
                                                doping_source_init, doping_drain_init, doping_channel_init, mark_list)
    ...
    for ee in range(len(E_list_poi)):
        dos[ee] = [dos0[ee]] * num_cell
    dos_GP_list = constant_parameters.get_dos_GP_list(coord_GP_list, dos, z_total, mark_list)

    # compute phi_s and phi_d with physical doping
    phi_l = 0
    phi_r = 0
    # phi_l = - mul + ef_solver.brent("n", zero_index, E_list_poi, E_list_n, E_step_poi, Ec, Eg,
    #                               0, dos0, doping_source,
    #                               E_list_poi[0]-10, E_list_poi[len(E_list_poi) - 1]+10, 0, 1e-4)
    # phi_r = - mur + ef_solver.brent("n", zero_index, E_list_poi, E_list_n, E_step_poi, Ec, Eg,
    #                               0, dos0, doping_drain,
    #                               E_list_poi[0]-10, E_list_poi[len(E_list_poi) - 1]+10, 0, 1e-4)
    # print("physical doping phi_l:", phi_l)
    # print("physical doping phi_l:", phi_r)
    ...
    # compute the parameters of effective dos
    q = int(chirality[0] * 2 / 3 + 1 / 3)
    Evh1 = abs(2.97 * (1 + 2 * np.cos(q * np.pi / chirality[0])))
    Evh2 = abs(2.97 * (1 - 2 * np.cos(q * np.pi / chirality[0])))
    alpha = 1
    a = 2.46
    Nc = alpha * (2 * Evh1 + KT) * np.sqrt(2 * KT / (3 * np.pi * Evh1)) / (a * Evh2) * 2 * L / volume_cell
    Nv = Nc

    phi_guess = poissonGuess(1, info_mesh, cell_long_term, cell_NJ, Dirichlet_list, Dirichlet_BC, dof_amount,
                             doping_GP_list, fixedCharge_GP_list, geo_para, phi_l, phi_r, Dirichlet_BC_gate)
    ...
    while iter_big < iter_big_max:
        ...
        TOL_ef = 1e-4
        TOL_du = 1e-4
        iter_NonLinearPoisson_max = 50
        phi = poisson(1, info_mesh, N_GP_T, cell_long_term, cell_NJ, cell_NNTJ, cnt_cell_list,
