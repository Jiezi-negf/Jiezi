...
def rgf(ee, E_list, eta, mul, mur, Hii, Hi1, Sii, sigma_lesser_ph, sigma_r_ph):
    ...
    g_0_lesser = op.trimatmul(g_0_R,
                              op.addmat(op.matmulmat(sigma_lesser_ph[ee][0], Sii[0]), Sigma_left_lesser),
                              g_0_R, type="nnc")
    g_lesser.append(g_0_lesser)

    # 4 derive little retarded GF from i=1 to i=nz-2  -- g_i_R
    for i in range(1, nz - 1):
        g_i_R = op.inv(op.addmat(op.scamulmat(w, Sii[i]),
                                 Hii[i].nega(),
                                 op.matmulmat(sigma_r_ph[ee][i], Sii[i]).nega(),
                                 op.trimatmul(Hi1[i], g_R[i - 1], Hi1[i], type="cnn").nega()))
        g_R.append(g_i_R)

    # 5 derive little lesser GF from i=1 to i=nz-2  -- g_i_lesser
    for i in range(1, nz - 1):
        g_i_lesser = op.trimatmul(g_R[i],
                                  op.addmat(op.matmulmat(sigma_lesser_ph[ee][i], Sii[i]),
                                            op.trimatmul(Hi1[i], g_lesser[i - 1], Hi1[i], type="cnn")),
                                  g_R[i], type="nnc")
    ...
    Sigma_right_lesser = op.scamulmat(fermi(E_list[ee] - mur),
                                      op.scamulmat(complex(0, 1),
                                                   op.addmat(Sigma_right_R, Sigma_right_R.dagger().nega())))
    Sigma_right_greater = op.scamulmat(1 - fermi(E_list[ee] - mur),
                                       op.scamulmat(complex(0, 1),
                                                    op.addmat(Sigma_right_R, Sigma_right_R.dagger().nega())))
    ...
    #     G_R.append(temp)
    G_R = [None] * nz
    G_R[nz - 1] = op.inv(op.addmat(op.scamulmat(w, Sii[nz - 1]), Hii[nz - 1].nega(),
                                   op.matmulmat(sigma_r_ph[ee][nz - 1], Sii[nz - 1]).nega(),
                                   op.trimatmul(Hi1[nz - 1], g_R[nz - 2], Hi1[nz - 1], type="cnn").nega(),
                                   Sigma_right_R.nega()))
    ...
    G_lesser = [None] * nz
    G_lesser[nz - 1] = op.trimatmul(G_R[nz - 1],
                                    op.addmat(op.matmulmat(sigma_lesser_ph[ee][nz - 1], Sii[nz - 1]),
                                              op.trimatmul(Hi1[nz - 1], g_lesser[nz - 2], Hi1[nz - 1], type="cnn"),
                                              Sigma_right_lesser),
                                    G_R[nz - 1], type="nnc")

    # 9.1 derive real retarded GF from i=nz-2 to i=0  -- G_i_R
    for i in range(nz - 2, -1, -1):
        G_R[i] = op.addmat(g_R[i],
                           op.trimatmul(g_R[i],
                                        op.trimatmul(Hi1[i + 1], G_R[i + 1], Hi1[i + 1], type="nnc"),
                                        g_R[i], type="nnn"))

    # 9.2 derive real lesser GF from i=nz-2 to i=0  -- G_i_lesser
    for i in range(nz - 2, -1, -1):
        G_lesser[i] = op.addmat(g_lesser[i],
                                op.trimatmul(g_lesser[i],
                                             op.trimatmul(Hi1[i + 1], G_R[i + 1], Hi1[i + 1], type="ncc"),
                                             g_R[i], type="nnc"),
                                op.trimatmul(g_R[i],
                                             op.trimatmul(Hi1[i + 1], G_lesser[i + 1], Hi1[i + 1], type="nnc"),
                                             g_R[i], type="nnc"),
                                op.trimatmul(g_R[i],
                                             op.trimatmul(Hi1[i + 1], G_R[i + 1], Hi1[i + 1], type="nnc"),
                                             g_lesser[i], type="nnn"))
    ...
    for i in range(0, nz):
        G_i_greater = op.addmat(G_lesser[i].nega(),
                                op.scamulmat(complex(0, 1),
                                             op.addmat(G_R[i], G_R[i].dagger().nega())
                                             )
                                )
